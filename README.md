...
运行后：
- 控制台会显示启动日志；
- 打开浏览器访问 `http://localhost:3000` 可以实时看到渲染器输出（采用 Server-Sent Events）。
...
- `sample-app/app.js`：示例小程序，定期调用 `setData`。

说明：此项目仅作学习与演示用途，非生产级实现。你可以在 `src/runtime.js` 中扩展更多 API（如网络、storage、事件回调等），并把 `renderer` 换成一个真实的原生/浏览器渲染层。
