...
在正常web中，js代码是会阻塞页面渲染的，这在一定程度上影响了页面在感官上的响应速度。而微信小程序采用双线程模型，即渲染层线程与逻辑层线程独立运行，通过原生层中转，保留了Web技术的灵活性，又发挥了原生应用的性能优势，后面各大厂商推出的小程序也基本采取这一方式。
...
8. 数据变更后，`app.js`中调用setData方法，会触发更新事件，让`bridge.js`携带新的上下文通知 `renderer.js` 做更新。
...
5. **模板解析渲染**：通过htmlparser2编译模板为AST,实现简易的WXML模板解析器实现数据绑定和指令处理
